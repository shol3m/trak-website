...
- `components/ui/ProductCard.tsx` — карточка товара, поддержка FeaturedProduct и MockProduct, onAddToCart. Prop `theme` удалён — цвета через семантические токены. Бренд — текстом рядом с артикулом (не плашка поверх фото, убрано 2026-08-04). Бейдж «Под заказ» — янтарный (`amber`), не серый (был неразличим на глаз)
...
- `app/product/[article]/page.tsx` — страница товара (галерея, артикул, цена, полный breadcrumb через `getCategoryNode()`). Раньше жила на `/catalog/[slug]/[article]`, переехала на верхний уровень — артикул глобально уникален, слаг категории в URL не нужен. Бренд — кликабельный кикер над `<h1>` (ссылка на `/catalog?brand=...`), не мелкая красная плашка. Блок «Гарантия» — нейтральный текст без придуманных сроков. Колонка под фото — фикс 380px (`md:grid-cols-[380px_1fr]`), не половина экрана. Условия доставки сознательно не добавлены (заказчик попросил не объявлять, пока не готовы), применяемость по авто — нет данных (`ProductCompatibility` пустая)
...
- **Кикер-лейблы** (мелкий разреженный uppercase-текст над заголовком секции — «Помощь эксперта», «Автосервис», «Запись» и т.п., см. `SectionHeading.tsx` и повторения в `BookingModal`/`PartRequestModal`/`ServiceBookingCTA`/`PartFinderCTA`/`HeroSlider`/`ServiceSection`/`about`/`contacts`) — были на `tracking-[0.2em]`, узнаваемый штамп шаблонных AI-лендингов. Сужено до `tracking-widest` (0.1em) 2026-08-04. **Заказчик повторно отметил этот стиль как «иишный»** тем же днём после сужения трекинга — при следующей правке рассмотреть отказ от самого паттерна «мелкий кикер над заголовком», не просто дальше сужать трекинг (см. «ЧТО НЕ РЕАЛИЗОВАНО»)
...
### Приоритет 0.5 — дизайн, план от заказчика (2026-08-04, ждёт реализации)
Заказчик прислал скриншоты и продиктовал план после P12. Ничего из этого списка ещё не сделано — только записано.
- **Кикер-лейблы всё ещё «иишные»** даже после сужения трекинга (P12) — заказчик хочет другой стиль целиком, не просто уже трекинг. Нужно обсудить конкретное направление (убрать паттерн «мелкий uppercase над заголовком» вообще? другой регистр/размер/цвет?) — не уточнено, спросить перед реализацией
- **Каталог в футере неправильный** (`components/layout/Footer.tsx`) — хардкод 5 ссылок («Двигатели», «Фильтры», «Тормоза», «Подвеска», «Масла»), все ведут на `/catalog` без категории — это старые 7 flat-категорий, которых с 2026-08-01 нет в реальном дереве (см. «Slug ↔ категория»). Нужно взять реальные корневые категории через `getCategoryTree()`
- **Корзина**: убрать emoji, заменить на SVG-иконки — `CartDrawer.tsx` содержит 🛒 (пустая корзина, строка ~182) и ✅ (экран «Заказ принят», строка ~268). P5 (2026-04-25) убирал только 📦-плейсхолдер товара, эти два остались незамеченными
- **Каталог — инлайн-степпер количества**: после клика «В корзину» на карточке товара показывать под товаром счётчик (1, 2...) вместо того чтобы снова показывать кнопку «В корзину» при повторном добавлении. Нужен пропс/стейт на `ProductCard.tsx` (и `CatalogView.tsx` list-режим), синхронизированный с `useCartStore`
- **Страница товара — полный редизайн**: заказчик считает текущий вид (`app/product/[article]/page.tsx`) некрасивым. В частности кнопка «Добавить в корзину» намного крупнее, чем нужно — уменьшить. Остальное — открытый вопрос, обсудить направление перед версткой
- **Отдельная страница корзины** — сейчас только `CartDrawer` (слайд-панель). Нужна полноценная `/cart` (или похожий роут), не просто drawer
- **Убрать формулировку об адресе доставки из поля комментария в корзине** (`CartDrawer.tsx`) — не актуально. Раньше (см. P11-соседний пункт «Доставка убрана везде») это поле сознательно не трогали, посчитав его рабочим полем формы, а не маркетинговым текстом — заказчик уточнил, что и это поле нужно поправить
