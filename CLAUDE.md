...
- `components/ui/BrandSelect.tsx` — выпадающая панель выбора марки/бренда с полем живого поиска (заменила нативный `<select>` на 200+ опций, 2026-08-04), группы «Марки техники»/«Бренды запчастей», используется в `CatalogView.tsx`
...
- **Множественный выбор марки/бренда** в фильтре каталога (сейчас можно выбрать только один за раз) — план согласован (URL `?brand=A,B`, `.in()` вместо `.eq()`, чекбоксы в `BrandSelect.tsx`), не реализовано
