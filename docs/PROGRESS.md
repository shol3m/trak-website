...
4. **CategoriesSection** — убрать с главной (`app/page.tsx`). После фикса п.2 дублирует те же категории + в `/catalog` уже есть полноценная сетка (`CategoryTiles`). Секция `py-20`, 6 плиток на фейковых категориях — много места, мало пользы. Файл `components/sections/CategoriesSection.tsx` оставить неиспользуемым или удалить.
5. **ProductsSection → замена** — `getFeaturedProducts(4)` не «популярное», а просто последние засинканные товары с остатком; фото пока плейсхолдеры (реальных от 1С нет). Заменить на блок **«Эксперт поможет подобрать запчасть»** (топ-идея с rossko.ru): телефон + свободное поле «какая деталь нужна» (для тех, кто не знает артикул/название) + опционально марка/модель авто. Технически: новая секция-компонент + модалка по образцу `BookingModal` (переиспользовать `lib/phone-utils.ts`, паттерн маски телефона/rate-limit/honeypot), либо расширить `/api/booking` полем комментария вместо нового API-роута.
6. **ServiceSection** — без изменений, признан рабочим.
7. **ContactsSection global duplicate** — убрать `<ContactsSection />` из `app/layout.tsx:100` (рендерится на каждой странице поверх Footer, дублирует полноценную `/contacts`, которая уже не использует этот компонент с сессии 2026-05-25). После удаления `components/sections/ContactsSection.tsx` нигде не используется — удалить файл.
