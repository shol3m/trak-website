...
## Реальное дерево категорий из справочника 1С (2026-08-01)
...
- `lib/categories.ts` — удалена keyword-детекция (`detectCategorySlug`, `CATEGORY_KEYWORDS`), она больше не нужна. `STATIC_CATEGORIES` оставлен — используется только для витрины на главной (`CategoriesSection`, `PartFinderSection`)
- `lib/db-catalog.ts` — `getCategories()` теперь возвращает только категории, у которых есть хотя бы один активный товар (inner join с `Product`) — иначе таб-бар каталога показывал бы сотни пустых категорий из нового дерева
- `app/catalog/[slug]/page.tsx` — снят хардкод `STATIC_CATEGORIES`/`generateStaticParams` (7 слугов), валидность слага теперь проверяется по факту наличия категории с товарами в БД
- `app/catalog/[slug]/[article]/page.tsx` — хлебные крошки берут категорию из самого товара, а не из статичного списка

### Важно: существующие 280k товаров пока не перекатегоризированы

`Код_Каталога` раньше нигде не сохранялся, поэтому у существующих товаров в БД нет данных для сопоставления с новым деревом — они остаются в старых 7 категориях. Новая логика применится автоматически к товарам, которые придут при следующей синхронизации от 1С (POST `/api/products` уже шлёт `Код_Каталога`, просто раньше он отбрасывался). Ручной re-import CSV не потребовался.
