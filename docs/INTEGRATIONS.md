...
| Код_Каталога | игнорируется (ждём справочник категорий) |
...
**Логика категорий:** `detectCategorySlug(name)` в `lib/categories.ts` — keyword-маппинг по Наименованию → один из 7 slug. Категория создаётся в БД автоматически при первом товаре.
...
### Категории от 1С (ожидается)
Текущие 7 категорий — приблизительные, определяются по ключевым словам в `lib/categories.ts`.
Когда придёт справочник `Код_Каталога → Название` от 1С-разработчика:
- Обновить `lib/categories.ts`
- Написать SQL для перекатегоризации существующих 280k товаров
